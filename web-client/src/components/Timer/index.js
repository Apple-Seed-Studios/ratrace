...
const Timer = function () {
  ...
  let handleTimerSubmit = (e) => {
    ...
    let payload = {
      workTime: convertTimeMilliseconds(
        parseInt(splitWork[0]),
        parseInt(splitWork[1])
      ),
      restTime: convertTimeMilliseconds(
        parseInt(splitRest[0]),
        parseInt(splitRest[1])
      ),
      workCycle: workCycle,
    };
      ...
      <Box sx={{ marginBottom: 10 }}>
        {toggleEdit ? (
          <>
            <form onSubmit={handleTimerSubmit}>
              <TextField
                ...
                label="Work Time"
                defaultValue={
                  convertTimeReadable(time.defaultWork).minutesSeconds
                }
              />
              <TextField
                {...textFieldPropsRest}
                ...
                label="Rest Time"
                defaultValue={
                  convertTimeReadable(time.defaultRest).minutesSeconds
                }
              />
              <Button
                disabled={textFieldPropsWork || textFieldPropsRest}
                type="submit"
              >
                submit
              </Button>
          ...
          <Typography variant="h3">
            Work Cycle: {readableTime.minutesSeconds}
          </Typography>
        ) : (
          <Typography variant="h3">
            Rest Cycle: {readableTime.minutesSeconds}
          </Typography>
        )}
        <Fab onClick={handleToggle} size="small">
          <EditIcon />
        </Fab>
        {timerOn ? (
          <Fab size="small" onClick={stopTimer}>
            <PauseIcon />
          </Fab>
        ) : (
          <Fab size="small" onClick={startTimer}>
            <PlayArrowIcon />
          </Fab>
        )}
        <Fab size="small" onClick={() => skip()}>
          <NextPlanIcon />
        </Fab>
      </Box>
    </>
